...
def booksWithdrawal():
    ...
    accessionNo_field.place(relx=0.65, rely=0.4, width = 660, height = 40, anchor="center")
...
def popup_window():
    ...
    if (len(in_database) < 1):
        ...
        button1.config(font=(FONT, 15, STYLE))
        button1.place(relx=0.3, rely=0.85, anchor="center")
        button2 = Button(win1, text = "Back to Withdrawal Function", bg = "#5AA9E6", command = win1.destroy)
        button2.config(font=(FONT, 15, STYLE))
        button2.place(relx=0.7, rely=0.85, anchor="center")
...
def withdraw_books():
    ...
    if (len(on_loan) > 0):
        ...
        label2.place(relx=0.5, rely=0.5, anchor="center")
        label2.config(font=(FONT, FONT_SIZE, STYLE))
        btn = Button(win, text = "Return to Withdrawal Function", bg = "#5AA9E6", command = lambda: [win.destroy(), win1.destroy()])
        btn.place(relx=0.5, rely=0.9, anchor="center")
        btn.config(font=(FONT, FONT_SIZE, STYLE))
