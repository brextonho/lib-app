...
## to do try-except IntegrityError
...
def return_books():
    ...
    if (fineAmount > 0):
        ...
        win.title("Error")
        label1 = Label(win2, text = "Error!", bg = "#eb1e1e")
        label1.place(relx=0.5, rely=0.1, anchor="center")
        label1.config(font=(FONT, MEDIUM_FONT_SIZE, STYLE))
        label2 = Label(win2, text = "Book returned successfully but has fines.", bg = "#eb1e1e")
        label2.place(relx=0.5, rely=0.5, anchor="center")
        label2.config(font=(FONT, MEDIUM_FONT_SIZE, STYLE))
        btn = Button(win2, text = "Back to Return Function", bg = "#5AA9E6", command = win2.destroy)
        btn.place(relx=0.5, rely=0.9, anchor="center")
        btn.config(font=(FONT, MEDIUM_FONT_SIZE, STYLE))
    else:
        # Success
        win.geometry("800x400")
        ...
        label1 = Label(win2, text = "Success!", bg = "#b0f556")
        label1.place(relx=0.5, rely=0.1, anchor="center")
        label1.config(font=(FONT, MEDIUM_FONT_SIZE, STYLE))
        label2 = Label(win2, text = "Book returned successfully.", bg = "#b0f556")
        label2.place(relx=0.5, rely=0.5, anchor="center")
        label2.config(font=(FONT, MEDIUM_FONT_SIZE, STYLE))
        btn = Button(win2, text = "Back to Return Function", bg = "#5AA9E6", command = win2.destroy)
        btn.place(relx=0.5, rely=0.9, anchor="center")
...
def popup_window():
    ...
    if (len(book_valid) > 0):
        ...
        button2 = Button(win, text = "Back to Return Function", bg = "#5AA9E6", command = lambda: [win.destroy(), win2.destroy()])
        button2.config(font=(FONT, MEDIUM_FONT_SIZE, STYLE))
        ...
        label2.place(relx=0.5, rely=0.5, anchor="center")
        label2.config(font=(FONT, MEDIUM_FONT_SIZE, STYLE))
        btn = Button(win, text = "Back to Return Function", bg = "#5AA9E6", command = lambda: [win.destroy(), win2.destroy()])
        btn.place(relx=0.5, rely=0.9, anchor="center")
        btn.config(font=(FONT, MEDIUM_FONT_SIZE, STYLE))
